...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        if [ $INITIAL_COPYRIGHT_YEAR -lt 2016 ]; then
            printf "$COMMENT_PREFIX Copyright (C) $INITIAL_COPYRIGHT_YEAR-2016 The PixelDust Project\n" > $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq 2016 ]; then
            printf "$COMMENT_PREFIX Copyright (C) 2016 The PixelDust Project\n" > $1
        fi
        if [ $YEAR -eq 2017 ]; then
            printf "$COMMENT_PREFIX Copyright (C) 2017 The PixelDust Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "$COMMENT_PREFIX Copyright (C) $YEAR The PixelDust Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "$COMMENT_PREFIX Copyright (C) 2017-$YEAR The PixelDust Project\n" >> $1
        else
            printf "$COMMENT_PREFIX Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The PixelDust Project\n" >> $1
        fi
    else
        printf "$COMMENT_PREFIX Copyright (C) $YEAR The PixelDust Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        ...
        export SMALIJAR="$PIXELDUST_ROOT"/vendor/pixeldust/build/tools/smali/smali.jar
    fi
