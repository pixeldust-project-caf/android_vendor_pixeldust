...
func init() {
	android.RegisterModuleType("pixeldust_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = pixeldustExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = pixeldustExpandVariables(ctx, dep_file)
	...
	manifestName := "pixeldust.sbox.textproto"
	desc := "generate"
	name := "generator"
