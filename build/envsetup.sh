...
cat <<EOF
Additional PixeldustROM functions:
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/pixeldust-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.pixeldust.device | grep -q "$PIXELDUST_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $PIXELDUST_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    if (adb shell getprop ro.pixeldust.device | grep -q "$PIXELDUST_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        pixeldustremote
        git push pixeldust HEAD:refs/heads/'$1'
    fi
    '
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.pixeldust.device | grep -q "$PIXELDUST_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $PIXELDUST_FIXUP_COMMON_OUT ]; then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/pixeldust/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
